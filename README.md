...
- `acceptance/test-repo-standard.mjs` — the standing lint. Checks: Keep a Changelog categories
  ...
  reflexivity, stable meta files, no surviving scaffold TODOs, and license agreement (a root
  license file exists, and a recognizable license text must match the manifest's license field
  and the README's License section). Exit 1 means the docs drifted; exit 2 means the config is
  broken; every skip is a printed, named line.
